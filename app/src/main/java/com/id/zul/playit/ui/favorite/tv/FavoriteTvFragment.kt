...
class FavoriteTvFragment : Fragment() {
    ...
    private fun emptyViews() {
        view_empty_tv.visibility = View.VISIBLE
        rv_favorite_tv.visibility = View.GONE
    }

    private fun setViews() {
        view_empty_tv.visibility = View.GONE
        rv_favorite_tv.visibility = View.VISIBLE
    }

    private fun setShimmer() {
        shimmer = Skeleton.bind(rv_favorite_tv)
            .adapter(adapter)
    ...
    private fun setRecyclerView(view: View) {
        ...
        if (activity!!.resources.configuration.orientation == Configuration.ORIENTATION_LANDSCAPE)
            rv_favorite_tv.layoutManager = GridLayoutManager(context, 4)
        else
            rv_favorite_tv.layoutManager = GridLayoutManager(context, 2)
